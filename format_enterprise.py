...
def format_enterprise_characteristics():
    """
    ...
    writer = csv.writer(open('data/enterprise_new.csv', 'w'))
...
def test():
    ...
    writer = csv.writer(open('data/PGDP-new.csv', 'w'))
    for row in reader:
        row[1] = row[1].strip()
        writer.writerow(row)


# format_enterprise_characteristics()
test()
